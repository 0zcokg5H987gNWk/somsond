...
# 5. Otomatis tangani perguso.py / ferguso.py dan ubah LOCAL_PORT menjadi 443
RUN if [ -f perguso.py ]; then \
        sed -i "s/LOCAL_PORT = 50512/LOCAL_PORT = 443/g" perguso.py; \
    elif [ -f ferguso.py ]; then \
        sed -i "s/LOCAL_PORT = 50512/LOCAL_PORT = 443/g" ferguso.py && cp ferguso.py perguso.py; \
    fi

# 6. Buat file model.toml otomatis (LocalPort diarahkan ke 443, RemotePort 50880)
